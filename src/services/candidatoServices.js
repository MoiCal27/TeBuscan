...
export const crearUsuarioCandidato = async (
  correo_usuario,
  password_usuario,
) => {
...
export const getEstadisticasCandidato = async (id_candidato) => {
  ...
  const aplicacionesMes =
    aplicaciones?.filter((a) => new Date(a.fecha_aplicacion) >= hoy).length ||
    0;

  const entrevistas =
    aplicaciones?.filter(
      (a) => a.evaluacion_aplicacion?.[0]?.estado_aplicacion === "Entrevista",
    ).length || 0;
...
export const aplicarEmpleo = async (
  id_candidato,
  id_empleo,
  mensaje_aplicacion,
) => {
