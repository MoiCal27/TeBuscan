...
export const getTodosLosEmpleos = async ({
  categoria,
  experiencia,
  contrato,
  salario_min,
...
export const getValoracionesEmpresa = async (id_empresa) => {
  ...
  const promedio =
    data && data.length > 0
      ? (
          data.reduce((acc, v) => acc + v.calificacion, 0) / data.length
        ).toFixed(1)
      : 0;
...
export const getCategoriasRecursos = async () => {
  const { data, error } = await supabase.schema("tebuscan").from("recursos")
    .select(`
            id_recurso,
            categoria (
                id_categoria,
...
export const getTodasLasEmpresas = async ({
  busqueda,
  industria,
  tamano,
} = {}) => {
  ...
  return data.map((emp) => ({
    ...emp,
    empleos_activos: (emp.empleos || []).filter(
      (e) => e.estado_empleo === "activo",
    ).length,
    empleos: undefined,
  }));
};
...
export const getValoracionesPorEmpresa = async (id_empresa) => {
  ...
  const promedio =
    total > 0
      ? (
          data.reduce((acc, v) => acc + Number(v.calificacion), 0) / total
        ).toFixed(1)
      : 0;
...
// ─────────────────────────────────────────────────────────────
// AÑADIR al final de generalServices.js
// ─────────────────────────────────────────────────────────────
