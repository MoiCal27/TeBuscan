import { getEmpleos, crearEmpleo, actualizarEmpleo } from '../api/empresaApi.js';
...
export async function cargarEmpleos() {
    try {
        ...
        if (!empleos || empleos.length === 0) {
            contenedor.innerHTML = '<p class="text-muted small">No hay empleos publicados aún.</p>';
            return;
        }

        contenedor.innerHTML = empleos.map(emp => `
...
export async function guardarNuevoEmpleo() {
    const campos = [
        'pub-titulo', 'pub-ubicacion', 'pub-categoria',
        'pub-tipo', 'pub-contrato', 'pub-experiencia',
        'pub-descripcion', 'pub-requisitos'
    ];
    limpiarTodos(campos);
    let valido = true;
    ...
    if (!titulo) { mostrarError('pub-titulo', 'El título es obligatorio'); valido = false; }
    if (!ubicacion) { mostrarError('pub-ubicacion', 'La ubicación es obligatoria'); valido = false; }
    if (!categoria) { mostrarError('pub-categoria', 'Selecciona una categoría'); valido = false; }
    if (!tipo) { mostrarError('pub-tipo', 'Selecciona el tipo de empleo'); valido = false; }
    if (!contrato) { mostrarError('pub-contrato', 'Selecciona el tipo de contrato'); valido = false; }
    if (!experiencia) { mostrarError('pub-experiencia', 'Selecciona el nivel de experiencia'); valido = false; }
    if (!descripcion) { mostrarError('pub-descripcion', 'La descripción es obligatoria'); valido = false; }
    if (!requisitos) { mostrarError('pub-requisitos', 'Los requisitos son obligatorios'); valido = false; }
    ...
    try {
        const data = {
            ...
            salario_min_empleo: document.getElementById('pub-salario_min').value || null,
            salario_max_empleo: document.getElementById('pub-salario_max').value || null,
...
export async function guardarEdicionEmpleo() {
    const campos = [
        'edit-titulo', 'edit-ubicacion', 'edit-categoria',
        'edit-tipo', 'edit-contrato', 'edit-experiencia',
        'edit-descripcion', 'edit-requisitos'
    ];
    limpiarTodos(campos);
    let valido = true;
    ...
    if (!titulo) { mostrarError('edit-titulo', 'El título es obligatorio'); valido = false; }
    if (!ubicacion) { mostrarError('edit-ubicacion', 'La ubicación es obligatoria'); valido = false; }
    if (!categoria) { mostrarError('edit-categoria', 'Selecciona una categoría'); valido = false; }
    if (!tipo) { mostrarError('edit-tipo', 'Selecciona el tipo de empleo'); valido = false; }
    if (!contrato) { mostrarError('edit-contrato', 'Selecciona el tipo de contrato'); valido = false; }
    if (!experiencia) { mostrarError('edit-experiencia', 'Selecciona el nivel de experiencia'); valido = false; }
    if (!descripcion) { mostrarError('edit-descripcion', 'La descripción es obligatoria'); valido = false; }
    if (!requisitos) { mostrarError('edit-requisitos', 'Los requisitos son obligatorios'); valido = false; }
    ...
    try {
        const data = {
            ...
            tipo_empleo: tipo,
            tipo_contrato_empleo: contrato,
            salario_min_empleo: document.getElementById('edit-salario_min').value || null,
            salario_max_empleo: document.getElementById('edit-salario_max').value || null,
            nivel_experiencia_empleo: experiencia,
            descripcion_empleo: descripcion,
            requisitos_empleo: requisitos,
            beneficios_empleo: document.getElementById('edit-beneficios').value.trim(),
        };
...
window.abrirEditar = async (id_empleo) => {
    ...
    document.getElementById('edit-contrato').value = emp.tipo_contrato_empleo || '';
    document.getElementById('edit-salario_min').value = emp.salario_min_empleo || '';
    document.getElementById('edit-salario_max').value = emp.salario_max_empleo || '';
    document.getElementById('edit-experiencia').value = emp.nivel_experiencia_empleo || '';
    document.getElementById('edit-descripcion').value = emp.descripcion_empleo || '';
    document.getElementById('edit-requisitos').value = emp.requisitos_empleo || '';
    document.getElementById('edit-beneficios').value = emp.beneficios_empleo || '';
