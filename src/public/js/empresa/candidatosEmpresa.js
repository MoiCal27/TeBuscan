...
function mostrarListaCandidatos(candidatos) {
  ...
  contenedor.innerHTML = candidatos
    .map((apl) => {
      ...
      const tituloProfesional =
        c.candidato_x_titulo?.[0]?.titulo?.nombre_titulo || "";
...
window.abrirModalCandidatoData = function (apl) {
  ...
  const nombreCompleto = `${c.nombre_candidato} ${c.apellido_candidato}`;
  const tituloProfesional =
    c.candidato_x_titulo?.[0]?.titulo?.nombre_titulo || "Sin título";
...
window.cambiarEstado = function (btn) {
  document
    .querySelectorAll(".pec-btn-estado")
...
window.guardarEstadoCandidato = async function () {
  ...
  const notas_internas =
    document.getElementById("modal-cand-notas")?.value || "";

  try {
    await actualizarEstadoAplicacion(id_aplicacion, {
      estado_aplicacion,
      notas_internas,
    });
