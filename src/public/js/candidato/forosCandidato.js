...
function mostrarRespuestas(respuestas, id_foro) {
  const items = (respuestas || [])
    .filter((r) => r.estado_respuesta === "activo")
    .map(
      (r) => `
...
function mostrarPublicaciones(foros) {
  ...
  contenedor.innerHTML = forosActivos
    .map((f) => {
      const catNombre = f.categoria?.nombre_categoria || "";
      const badgeClass = CATEGORIAS[catNombre] || "carrera";
      const respuestas = (f.foro_respuesta || []).filter(
        (r) => r.estado_respuesta === "activo",
      );
...
async function cargarEstadisticas() {
  try {
    ...
    document.getElementById("stat-foro-hoy").textContent = estadisticas.hoy;
  } catch (error) {
...
window.mostrarPanelRespuestas = function (card) {
  ...
  document
    .querySelectorAll(".foro-respuestas-panel")
    .forEach((p) => (p.style.display = "none"));
  document
    .querySelectorAll(".foro-post-card")
    .forEach((c) => c.classList.remove("foro-post-card-open"));
  if (!visible) {
    ...
    incrementarVistas(id_foro).then((resultado) => {
      if (resultado?.message === "Vistas actualizadas") {
        const vistaSpan = card.querySelector(
          ".foro-post-footer span:last-child",
        );
        if (vistaSpan) {
...
window.enviarRespuesta = async function (id_foro) {
  ...
  try {
    ...
    mostrarToast(
      "Respuesta enviada",
      "Tu respuesta está pendiente de aprobación por el administrador.",
    );
  } catch (error) {
...
window.seleccionarOrden = function (btn) {
  document
    .querySelectorAll(".foro-ordenar-btn")
    .forEach((b) => b.classList.remove("active"));
  btn.classList.add("active");
  ...
  cards.sort((a, b) => {
    if (orden.includes("popular")) {
      const vistasA = parseInt(
        a
          .querySelector(".foro-post-footer span:last-child")
          ?.textContent.match(/\d+/)?.[0] || 0,
      );
      const vistasB = parseInt(
        b
          .querySelector(".foro-post-footer span:last-child")
          ?.textContent.match(/\d+/)?.[0] || 0,
      );
      ...
      const fechaA =
        a
          .querySelector(".foro-post-meta span:nth-child(3)")
          ?.textContent.trim() || "";
      const fechaB =
        b
          .querySelector(".foro-post-meta span:nth-child(3)")
...
document.querySelector(".foro-search-input").addEventListener("input", (e) => {
  ...
  document.querySelectorAll(".foro-post-card").forEach((card) => {
    const titulo =
      card.querySelector(".foro-post-title")?.textContent.toLowerCase() || "";
    const preview =
      card.querySelector(".foro-post-preview")?.textContent.toLowerCase() || "";
    card.style.display =
      titulo.includes(texto) || preview.includes(texto) ? "block" : "none";
  });
});
