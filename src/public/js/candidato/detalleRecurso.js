...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    document.getElementById("recurso-titulo").textContent =
      recurso.titulo_recurso;
    document.title = `TeBuscan - ${recurso.titulo_recurso}`;

    const fecha = recurso.fecha_recurso
      ? new Date(recurso.fecha_recurso).toLocaleDateString("es-ES", {
          day: "numeric",
          month: "long",
          year: "numeric",
        })
    ...
    document.getElementById("recurso-descripcion").textContent =
      recurso.descripcion || "";
    ...
    window.cambiarEstadoLike = async function () {
      ...
      btn.querySelector("i").className =
        `bi ${likeARecurso ? "bi-hand-thumbs-up-fill" : "bi-hand-thumbs-up"}`;
    ...
    if (
      contenido.includes("youtube.com/embed") ||
      contenido.includes("youtu.be")
    ) {
      ...
      lineas.forEach((l) => {
        if (l.startsWith("- ")) {
          if (!enLista) {
            html += '<ul style="padding-left:20px;margin-bottom:12px;">';
            enLista = true;
          }
    ...
    if (autor) {
      document.getElementById("autor-inicial").textContent =
        autor.nombre_autor?.charAt(0) || "?";
      document.getElementById("autor-nombre").textContent =
        autor.nombre_autor || "";
      document.getElementById("autor-rol").textContent = autor.rol_autor || "";
      document.getElementById("autor-desc").textContent =
        autor.descripcion || "";
    }
    ...
    const relacionados = recursos
      .filter(
        (r) =>
          r.id_recurso !== id &&
          r.categoria?.id_categoria === recurso.categoria?.id_categoria,
      )
      .slice(0, 3);

    document.getElementById("recursos-relacionados").innerHTML =
      relacionados.length
        ? relacionados
            .map(
              (r) => `
