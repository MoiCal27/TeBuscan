...
function registrarEventos() {
  let debounceTimer;
  document
    .getElementById("input-busqueda-empresa")
  ...
  document
    .getElementById("btn-limpiar-empresas")
...
function limpiarFiltros() {
  ...
  window.history.replaceState({}, "", "/buscar-empresas-candidato");
  buscar();
}
...
async function cargarEstadisticas() {
  try {
    ...
    document.getElementById("stat-empleos").textContent =
      estadisticas.empleosActivos;
    document.getElementById("stat-industrias").textContent =
      estadisticas.totalIndustrias;
  } catch (err) {
...
function actualizarContadoresFiltros(empresas) {
  ...
  empresas.forEach((emp) => {
    ...
    porIndustria[emp.industria_empresa] =
      (porIndustria[emp.industria_empresa] || 0) + 1;
  });
  ...
  empresas.forEach((emp) => {
    ...
    document
      .querySelectorAll(".filtro-tamanio-btn[data-value]")
      .forEach((btn) => {
...
function renderCard(emp) {
  ...
  const desc = emp.descripcion_empresa
    ? emp.descripcion_empresa.substring(0, 130) +
      (emp.descripcion_empresa.length > 130 ? "…" : "")
    : "";
  const empleosActivos = emp.empleos_activos || 0;

  return `
    <a href="/detalle-empresa-candidato?id=${emp.id_empresa}" class="text-decoration-none">
