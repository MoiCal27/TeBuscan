...
function registrarEventos() {
  ...
  document.querySelectorAll(".filtro-experiencia").forEach((cb) => {
    cb.addEventListener("change", () => {
      filtros.experiencia = [
        ...document.querySelectorAll(".filtro-experiencia:checked"),
      ].map((c) => c.value);
      buscar();
    });
  });
  ...
  document.querySelectorAll(".filtro-contrato").forEach((cb) => {
    cb.addEventListener("change", () => {
      filtros.contratos = [
        ...document.querySelectorAll(".filtro-contrato:checked"),
      ].map((c) => c.value);
      buscar();
    });
  });
  ...
  document.querySelectorAll(".filtro-salario").forEach((cb) => {
    cb.addEventListener("change", () => {
      ...
      if (!checked.length) {
        ...
        filtros.salario_min = Math.min(
          ...checked.map((c) => Number(c.dataset.min || 0)),
        );
        filtros.salario_max = Math.max(
          ...checked.map((c) => Number(c.dataset.max || 999999)),
        );
      }
      buscar();
    });
...
function limpiarFiltros() {
  ...
  // Cambio 1: ruta de candidato
  window.history.replaceState({}, "", "/buscar-empleos-candidato");
  buscar();
}
...
function filtrarEnCliente(empleos) {
  return empleos.filter((emp) => {
    if (
      ...
      !filtros.categorias.includes(emp.categoria_empleo)
    )
      return false;
    if (
      filtros.experiencia.length > 1 &&
      !filtros.experiencia.includes(emp.nivel_experiencia_empleo)
    )
      return false;
    if (
      filtros.contratos.length > 1 &&
      !filtros.contratos.includes(emp.tipo_contrato_empleo)
    )
      return false;
    return true;
  });
...
function renderCard(emp) {
  ...
  // Cambio 2: link apunta al detalle de candidato
  return `
    <a href="/detalle-empleo-candidato?id=${emp.id_empleo}" class="text-decoration-none">
