...
async function aplicar(id) {
    const mensaje = document.getElementById('mensaje-aplicacion')?.value.trim() || '';
...
function estrellas(calificacion, total = 5) {
    ...
    return '★'.repeat(llenas) + `<span style="color:#e2e8f0">${'★'.repeat(vacias)}</span>`;
}
...
function renderBeneficios(emp) {
    ...
    if (!el) return;
    if (!items.length) {
        el.innerHTML = '<p class="text-muted small">Sin beneficios especificados.</p>';
        return;
    }
...
function renderValoraciones(valoraciones, promedio, total) {
    ...
    listaEl.innerHTML = valoraciones.slice(0, 2).map(v => {
        const nombre = v.candidato
            ...
            : 'Empleado anónimo';
        const fecha = v.fecha_valoracion ? tiempoTranscurrido(v.fecha_valoracion) : '';
        return `
...
function mostrarCargando() {
    ...
    document.getElementById('job-descripcion').textContent = 'Cargando descripción…';
