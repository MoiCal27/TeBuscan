...
btnPassC.addEventListener('click', () => togglePass('password_candidato', btnPassC));
btnCpassC.addEventListener('click', () => togglePass('confirm_password_c', btnCpassC));
...
btnRegistroCandidato.addEventListener('click', async () => {
    limpiarTodos(['nombre_candidato','apellido_candidato','correo_candidato',
        'telefono_candidato','password_candidato','confirm_password_c']);
    let valido = true;

    const nombre_candidato = document.getElementById('nombre_candidato').value.trim();
    const apellido_candidato = document.getElementById('apellido_candidato').value.trim();
    const correo_usuario = document.getElementById('correo_candidato').value.trim();
    const telefono_candidato = document.getElementById('telefono_candidato').value.trim();
    ...
    if (!nombre_candidato) { mostrarError('nombre_candidato', 'El nombre es obligatorio'); valido = false; }
    else if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(nombre_candidato)) { mostrarError('nombre_candidato', 'Solo letras'); valido = false; }
    if (!apellido_candidato) { mostrarError('apellido_candidato', 'El apellido es obligatorio'); valido = false; }
    else if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(apellido_candidato)) { mostrarError('apellido_candidato', 'Solo letras'); valido = false; }
    if (!correo_usuario) { mostrarError('correo_candidato', 'El correo es obligatorio'); valido = false; }
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(correo_usuario)) { mostrarError('correo_candidato', 'Correo no válido'); valido = false; }
    if (!telefono_candidato) { mostrarError('telefono_candidato', 'El teléfono es obligatorio'); valido = false; }
    else if (!/^[0-9]{4}-[0-9]{4}$/.test(telefono_candidato)) { mostrarError('telefono_candidato', 'Formato: 0000-0000'); valido = false; }
    if (!password_usuario) { mostrarError('password_candidato', 'La contraseña es obligatoria'); valido = false; }
    else if (password_usuario.length < 6) { mostrarError('password_candidato', 'Mínimo 6 caracteres'); valido = false; }
    if (!confirm_password) { mostrarError('confirm_password_c', 'Confirma tu contraseña'); valido = false; }
    else if (password_usuario !== confirm_password) { mostrarError('confirm_password_c', 'Las contraseñas no coinciden'); valido = false; }

    if (!valido) return;

    try {
        const respuesta = await registrarCandidato({ nombre_candidato, apellido_candidato,
            correo_usuario, telefono_candidato, password_usuario });
        if (respuesta.error) { mostrarError('correo_candidato', respuesta.error); return; }
