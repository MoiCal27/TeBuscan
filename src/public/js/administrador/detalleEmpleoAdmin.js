...
async function cargarDetalle(id) {
    try {
        ...
        renderValoraciones(data.valoraciones, data.promedioValoracion, data.totalValoraciones);
...
function estrellas(calificacion, total = 5) {
    ...
    return '★'.repeat(llenas) + `<span style="color:#e2e8f0">${'★'.repeat(total - llenas)}</span>`;
}
...
function renderHeader(emp) {
    ...
    document.getElementById('job-empresa').textContent    = emp.empresa?.nombre_empresa || '';
    document.getElementById('job-ubicacion').textContent  = emp.ubicacion_empleo || 'No especificada';
    document.getElementById('job-tiempo').textContent     = tiempoTranscurrido(emp.creacion_empleo);
    document.getElementById('job-salario').textContent    = salario || 'No especificado';
    document.getElementById('job-tipo').textContent       = emp.tipo_empleo || 'No especificado';
    document.getElementById('job-experiencia').textContent = emp.nivel_experiencia_empleo || 'No especificado';
    document.getElementById('job-contrato').textContent   = emp.tipo_contrato_empleo || 'No especificado';

    const badgesEl = document.getElementById('job-badges');
    badgesEl.innerHTML = '';
    if (emp.categoria_empleo)      badgesEl.innerHTML += `<span class="badge-tech">${emp.categoria_empleo}</span>`;
    if (emp.estado_empleo === 'activo') badgesEl.innerHTML += `<span class="badge-active">Contratación activa</span>`;
}

function renderDescripcion(emp) {
    const el = document.getElementById('job-descripcion');
    if (el) el.textContent = emp.descripcion_empleo || 'Sin descripción disponible.';
}
...
function renderBeneficios(emp) {
    ...
    if (!el) return;
    el.innerHTML = items.length
        ? items.map(b => `<div class="benefit-item"><i class="bi bi-check2"></i> ${b}</div>`).join('')
...
function renderValoraciones(valoraciones, promedio, total) {
    ...
    listaEl.innerHTML = valoraciones.slice(0, 2).map(v => {
