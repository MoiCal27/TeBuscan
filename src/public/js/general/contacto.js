...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('btn-enviar-contacto')
...
async function enviarFormulario() {
    ...
    if (!nombre)  { mostrarError('input-nombre',  'El nombre es obligatorio.');         valido = false; }
    if (!correo)  { mostrarError('input-correo',  'El correo es obligatorio.');         valido = false; }
    if (correo && !correo.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
                   mostrarError('input-correo',  'Ingresa un correo válido.');           valido = false; }
    if (!asunto)  { mostrarError('input-asunto',  'El asunto es obligatorio.');         valido = false; }
    if (!mensaje) { mostrarError('input-mensaje', 'El mensaje no puede estar vacío.');  valido = false; }
    if (!valido) return;
    ...
    try {
        const resultado = await enviarMensajeContacto({ nombre, correo, asunto, mensaje });
